/* AI-901 Practice — static app, no build step, no accounts.
   Progress persists in localStorage under one key. */

(function () {
  "use strict";

  var STORE = "ai901.progress.v1";
  ...
  function load() {
    try {
      ...
      if (!raw) return { seen: {}, wrong: {}, runs: 0, best: null };
      var o = JSON.parse(raw);
      o.seen = o.seen || {}; o.wrong = o.wrong || {};
      o.runs = o.runs || 0;
      return o;
    } catch (e) { return { seen: {}, wrong: {}, runs: 0, best: null }; }
  }
  function save(p) {
  ...
  function render() {
    ...
    var f = el("div", "foot");
    f.innerHTML = "Progress is saved in this browser only — no account, no server. " +
      "Questions are original, written from the published AI-901 skills outline; " +
  ...
  function setupScreen() {
    var c = el("div", "card");
    c.appendChild(el("div", "eyebrow", "Azure AI Fundamentals"));
    c.appendChild(el("h1", null, "AI-901 Practice"));
    var p = el("p", "lede");
    p.textContent = QS.length + " original questions covering every area of the exam — Foundry hubs and " +
    ...
    topics().forEach(function (t) {
      ...
      mid.appendChild(el("span", "desc", TOPIC_DESC[t] || ""));
      b.appendChild(mid);
    ...
    // reset
    if (seenCount() > 0) {
      var rst = el("button", "chip small", "Reset all saved progress");
      rst.style.marginTop = "14px"; rst.style.color = "var(--sub)";
      rst.onclick = function () {
        if (confirm("Erase your saved progress in this browser? This cannot be undone.")) {
          prog = { seen: {}, wrong: {}, runs: 0, best: null };
          save(prog); render();
        }
      };
      c.appendChild(rst);
  ...
  function begin() {
    ...
    state.order.forEach(function (i) {
      state.optOrder[QS[i].id] = shuffle(QS[i].options.length);
    });
  ...
  function quizScreen() {
    ...
    head.appendChild(el("span", "mono small muted", "Q" + (state.idx + 1) + " / " + state.order.length));
  ...
  function resultsScreen() {
    ...
    c.appendChild(el("div", "eyebrow", "Results"));
